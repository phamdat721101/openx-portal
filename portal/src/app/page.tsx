...
export default function AgentStudioPage() {
  ...
  const [registration, setRegistration] = useState<{ agentId: string; agentKey?: string; restored?: boolean } | null>(null);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      {showConnect && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"><div className="w-full max-w-2xl rounded-2xl border border-outline-variant/30 bg-surface-container-low p-6 shadow-2xl"><div className="flex items-start justify-between"><div><h2 className="font-headline text-lg font-bold text-on-surface">Connect Agent</h2><p className="mt-1 text-xs text-on-surface-variant">Register once, then retain the agent ID and key in the agent host configuration.</p></div><button onClick={() => setShowConnect(false)}><X className="h-5 w-5 text-on-surface-variant" /></button></div>{registration ? <div className="mt-5 space-y-4"><p className="text-xs text-secondary">{registration.restored ? 'Agent restored. Resume its local sync scheduler; no new registration was created.' : 'Agent registered. Save this key now; it is shown once.'}</p><code className="block break-all rounded-xl bg-surface-container-high p-3 text-xs text-on-surface">OPENX_AGENT_ID={registration.agentId}{registration.agentKey ? `\nOPENX_AGENT_KEY=${registration.agentKey}` : ''}</code>{registration.agentKey && <button onClick={() => copyRegistrationValue(registration.agentKey!, 'key')} className="inline-flex items-center gap-1 text-xs font-bold text-primary"><Copy className="h-3.5 w-3.5" />{copiedRegistrationValue === 'key' ? 'Key copied' : 'Copy one-time key'}</button>}<div className="rounded-xl border border-outline-variant/30 bg-surface-container-high/50 p-3"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold text-on-surface">Agent setup prompt</p><p className="mt-1 text-[11px] text-on-surface-variant">This prompt includes your agent ID and gateway URL, but never the one-time key.</p></div><button onClick={() => copyRegistrationValue(buildAgentConnectionPrompt({ agentId: registration.agentId }), 'prompt')} className="shrink-0 inline-flex items-center gap-1 text-xs font-bold text-primary"><Copy className="h-3.5 w-3.5" />{copiedRegistrationValue === 'prompt' ? 'Prompt copied' : 'Copy setup prompt'}</button></div><pre className="mt-3 max-h-48 overflow-auto whitespace-pre-wrap rounded-lg bg-surface-container-lowest p-3 font-mono text-[10px] text-on-surface">{buildAgentConnectionPrompt({ agentId: registration.agentId })}</pre></div><p className="text-[11px] text-on-surface-variant">Schedule <code>python3 agent/sync_agent.py</code> every five minutes and run the agent worker for live task heartbeats. <Link href="/docs" className="font-bold text-primary">Open reusable integration docs</Link>.</p></div> : <div className="mt-5 space-y-5"><div className="space-y-3"><input value={displayName} onChange={(event) => setDisplayName(event.target.value)} className="w-full rounded-xl border border-outline-variant/40 bg-surface-container-high p-3 text-sm text-on-surface" placeholder="Agent name" /><div className="grid grid-cols-2 gap-3"><select value={hostType} onChange={(event) => setHostType(event.target.value as typeof hostType)} className="rounded-xl border border-outline-variant/40 bg-surface-container-high p-3 text-sm text-on-surface"><option value="adk-python">ADK Python</option><option value="claude-code">Claude Code</option><option value="kiro-cli">Kiro CLI</option><option value="custom">Custom</option></select><input value={model} onChange={(event) => setModel(event.target.value)} className="rounded-xl border border-outline-variant/40 bg-surface-container-high p-3 text-sm text-on-surface" placeholder="Model" /></div><button disabled={!gatewayOnline || registering || !displayName.trim()} onClick={connectAgent} className="w-full rounded-xl bg-primary px-4 py-3 text-xs font-bold text-on-primary disabled:opacity-50">{registering ? 'Registering…' : 'Register and issue key'}</button></div><div className="border-t border-outline-variant/20 pt-4"><p className="text-xs font-bold text-on-surface">Restore an existing agent</p><p className="mt-1 text-[11px] text-on-surface-variant">Use the agent ID and key already stored by a reinstalled host. This does not create a duplicate.</p><div className="mt-3 grid gap-3 sm:grid-cols-2"><input value={restoreAgentId} onChange={(event) => setRestoreAgentId(event.target.value)} className="rounded-xl border border-outline-variant/40 bg-surface-container-high p-3 text-sm text-on-surface" placeholder="OPENX_AGENT_ID" /><input value={restoreAgentKey} onChange={(event) => setRestoreAgentKey(event.target.value)} className="rounded-xl border border-outline-variant/40 bg-surface-container-high p-3 text-sm text-on-surface" placeholder="OPENX_AGENT_KEY" type="password" /></div><button disabled={!gatewayOnline || registering || !restoreAgentId.trim() || !restoreAgentKey.trim()} onClick={restoreAgent} className="mt-3 rounded-lg border border-primary/50 px-4 py-2 text-xs font-bold text-primary disabled:opacity-50">{registering ? 'Restoring…' : 'Restore existing agent'}</button></div></div>}</div></div>}
    </div>
  );
}
