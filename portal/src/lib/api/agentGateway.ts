...
export interface GraphTelemetry { provider: 'the_graph'; protocol: 'morpho_blue' | 'aave_v3'; chain_id: 'eip155:42161'; market_id: string; observed_at: string; fetched_at: string; status: 'ok' | 'degraded'; reason?: string; current_utilization_bps?: number; kink_utilization_bps?: number; kink_headroom_bps?: number; utilization_volatility_bps?: number; hourly_utilization_bps: number[]; }
export interface StatementReport { id: string; agent_id: string; report_id: string; content_hash: string; visibility: 'private' | 'public'; source_chain: string; source_block: string; source_timestamp: string; finality: 'finalized' | 'pending'; wallet_address?: string; venue: string; collateral_usd: number; debt_usd: number; realized_pnl_usd?: number; unrealized_pnl_usd?: number; pnl_methodology: string; ltv: number; recommended_ltv: number; max_ltv: number; status: 'received' | 'partial' | 'failed'; attestation: { status: 'pending' | 'verified' | 'unavailable_source_chain' | 'failed'; chain?: string; receipt?: string }; schema_version?: 'apsd-l/2.1'; the_graph_telemetry?: GraphTelemetry; oneinch_telemetry?: { status?: string; oracle_dex_divergence_bps?: number; divergence_status?: string; execution_allowed?: boolean }; pnl_attribution?: Record<string, string>; risk_engine_audit?: { risk_level?: string; health_factor_milli?: number }; actionable_allocation_vector?: { recommended_action?: string; target_ltv_bps?: number }; decision_context_card?: string; summary?: string; updated_at: string; }
export interface StatementContext { ok: true; agent_id: string; report_id: string; generated_at: string; freshness: { state: 'fresh' | 'stale' | 'unknown'; age_seconds?: number }; telemetry_status: 'ok' | 'degraded' | 'unavailable'; context_card: string; vector: { ltv_bps: number; health_factor_milli?: number; kink_headroom_bps?: number; utilization_volatility_bps?: number; risk_band?: string; recommended_action?: string }; verification: { statement_hash: string; attestation_status: string }; }
export interface StatementExecution { id: string; status: 'prepared' | 'submitted' | 'verified' | 'anchor_pending' | 'anchored' | 'failed'; attestation_status: 'pending_anchor' | 'pending_creditcoin_attestation' | 'verified' | 'unavailable' | 'failed'; receipt_hash?: string; arbitrum_tx_hash?: string; arbitrum_block?: string; ethereum_anchor_tx_hash?: string; ethereum_anchor_block?: string; creditcoin_chain_id?: string; creditcoin_chain_key?: string; creditcoin_block?: string; creditcoin_verified_at?: string; creditcoin_attested_height?: string; creditcoin_retry_at?: string; creditcoin_proof_tx_hash?: string; creditcoin_proof_tx_index?: string; creditcoin_proof_payload_hash?: string; creditcoin_proof_generated_at?: string; creditcoin_proof_cached?: boolean; creditcoin_verification_method?: string; creditcoin_explorer_url?: string; reason?: string; }
export interface AllocationExecution { id: string; status: 'prepared' | 'submitted' | 'verified' | 'failed'; usdc_collateral_amount: string; permit_mode: 'permit2' | 'approval'; transaction: { chainId: number; to: string; data: string; value: string }; arbitrum_tx_hash?: string; reason?: string; }
...
export async function fetchStatementContext(agentId: string): Promise<StatementContext | null> { try { const response = await fetch(`${GATEWAY_URL}/v1/agents/${encodeURIComponent(agentId)}/context/defi-lending`, { signal: AbortSignal.timeout(10_000) }); const body = await response.json(); return response.ok ? body as StatementContext : null; } catch { return null; } }
...
export async function fetchLatestStatementExecution(agentId: string): Promise<StatementExecution | null> { try { const res = await fetch(`${GATEWAY_URL}/v1/agents/${encodeURIComponent(agentId)}/statements/executions/latest`, { signal: AbortSignal.timeout(10_000) }); const body = await res.json(); return res.ok ? body.execution as StatementExecution | null : null; } catch { return null; } }
export async function retryStatementAttestation(agentId: string, executionId: string): Promise<StatementExecution | null> { try { const res = await fetch(`${GATEWAY_URL}/v1/agents/${encodeURIComponent(agentId)}/statements/executions/${encodeURIComponent(executionId)}/retry-attestation`, { method: 'POST', headers: { Accept: 'application/json' }, signal: AbortSignal.timeout(20_000) }); const body = await res.json(); return res.ok ? body.execution as StatementExecution : null; } catch { return null; } }
export async function prepareAllocation(agentId: string, reportId: string, walletAddress: string, usdcCollateralAmount: string): Promise<{ execution: AllocationExecution; approval_required: boolean; approval_transaction?: AllocationExecution['transaction'] } | null> { try { const res = await fetch(`${GATEWAY_URL}/v1/agents/${encodeURIComponent(agentId)}/statements/${encodeURIComponent(reportId)}/allocations/prepare`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ wallet_address: walletAddress, usdc_collateral_amount: usdcCollateralAmount }) }); return res.ok ? await res.json() : null; } catch { return null; } }
export async function submitAllocation(agentId: string, executionId: string, transactionHash: string): Promise<AllocationExecution | null> { try { const res = await fetch(`${GATEWAY_URL}/v1/agents/${encodeURIComponent(agentId)}/statements/allocations/${encodeURIComponent(executionId)}/submit`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ transaction_hash: transactionHash }) }); const body = await res.json(); return res.ok ? body.execution as AllocationExecution : null; } catch { return null; } }
export async function fetchLatestAllocation(agentId: string, reportId: string): Promise<{ configured: boolean; execution: AllocationExecution | null }> { try { const res = await fetch(`${GATEWAY_URL}/v1/agents/${encodeURIComponent(agentId)}/statements/${encodeURIComponent(reportId)}/allocations/latest`, { signal: AbortSignal.timeout(10_000) }); return res.ok ? await res.json() : { configured: false, execution: null }; } catch { return { configured: false, execution: null }; } }
export type FusionPreparation = { order_id: string; typed_data: Record<string, unknown>; target_ltv: number; expected_debt_repaid_usd: number; expires_at?: string };
export async function prepareStatementRebalance(agentId: string, reportId: string, walletAddress: string): Promise<FusionPreparation | null> { try { const res = await fetch(`${GATEWAY_URL}/v1/agents/${encodeURIComponent(agentId)}/statements/rebalance/prepare`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ report_id: reportId, wallet_address: walletAddress, target_ltv: .28, mode: 'fusion' }) }); const body = await res.json(); return res.ok ? body as FusionPreparation : null; } catch { return null; } }
export async function submitStatementRebalance(agentId: string, orderId: string, signature: string): Promise<boolean> { try { const res = await fetch(`${GATEWAY_URL}/v1/agents/${encodeURIComponent(agentId)}/statements/rebalance/submit`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ order_id: orderId, signed_order: { signature } }) }); return res.ok; } catch { return false; } }
