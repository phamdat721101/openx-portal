...
export interface StatementReport { id: string; agent_id: string; report_id: string; content_hash: string; visibility: 'private' | 'public'; source_chain: string; source_block: string; source_timestamp: string; finality: 'finalized' | 'pending'; wallet_address?: string; venue: string; collateral_usd: number; debt_usd: number; realized_pnl_usd?: number; unrealized_pnl_usd?: number; pnl_methodology: string; ltv: number; recommended_ltv: number; max_ltv: number; status: 'received' | 'partial' | 'failed'; attestation: { status: 'pending' | 'verified' | 'unavailable_source_chain' | 'failed'; chain?: string; receipt?: string }; schema_version?: 'apsd-l/2.1'; the_graph_telemetry?: { status?: string; current_utilization_bps?: number; kink_utilization_bps?: number; kink_headroom_bps?: number; utilization_volatility_bps?: number }; oneinch_telemetry?: { status?: string; oracle_dex_divergence_bps?: number; divergence_status?: string; execution_allowed?: boolean }; pnl_attribution?: Record<string, string>; risk_engine_audit?: { risk_level?: string; health_factor_milli?: number }; actionable_allocation_vector?: { recommended_action?: string; target_ltv_bps?: number }; decision_context_card?: string; summary?: string; updated_at: string; }
export interface StatementExecution { id: string; status: 'prepared' | 'submitted' | 'verified' | 'anchor_pending' | 'anchored' | 'failed'; attestation_status: 'pending_anchor' | 'pending_creditcoin_attestation' | 'verified' | 'unavailable' | 'failed'; receipt_hash?: string; arbitrum_tx_hash?: string; arbitrum_block?: string; ethereum_anchor_tx_hash?: string; ethereum_anchor_block?: string; creditcoin_chain_id?: string; creditcoin_block?: string; creditcoin_verified_at?: string; creditcoin_explorer_url?: string; reason?: string; }
export interface AllocationExecution { id: string; status: 'prepared' | 'submitted' | 'verified' | 'failed'; source_amount: string; min_collateral_amount: string; permit_mode: 'permit2' | 'approval'; transaction: { chainId: number; to: string; data: string; value: string }; arbitrum_tx_hash?: string; reason?: string; }
...
export async function prepareAllocation(agentId: string, reportId: string, walletAddress: string, sourceAmount: string): Promise<{ execution: AllocationExecution; approval_required: boolean } | null> { try { const res = await fetch(`${GATEWAY_URL}/v1/agents/${encodeURIComponent(agentId)}/statements/${encodeURIComponent(reportId)}/allocations/prepare`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ wallet_address: walletAddress, source_amount: sourceAmount, slippage_bps: 50 }) }); return res.ok ? await res.json() : null; } catch { return null; } }
